...
`theme.js` sets `color.legend: false` on purpose. Umbral labels series directly at the line end, and
that is also why `uncertainty.js`'s `label()` reaches for the `-text` variant — a label is small
text needing 4.5:1, while the line only needs 3:1.
